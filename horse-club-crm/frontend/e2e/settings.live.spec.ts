...
test('settings: administrator validates and saves the real club schedule', async ({ page, request }) => {
  ...
  try {
    ...
    await page.getByLabel('Выходные дни', { exact: true }).press('ArrowDown')
    await page.getByTitle('Воскресенье', { exact: true }).click()
    await page.getByLabel('Выходные дни', { exact: true }).press('ArrowDown')
    await page.getByTitle('Суббота', { exact: true }).click()
    ...
    expect(response.request().postDataJSON()).toEqual({ openTime: '08:30', closeTime: '22:15', daysOfWeekOff: [0, 6] })
    ...
    expect((await (await request.get(url, { headers })).json()).daysOfWeekOff).toEqual([0, 6])
    expect(errors).toEqual([])
  } finally {
    const restored = await request.patch(url, { headers, data: { openTime: original.openTime, closeTime: original.closeTime, daysOfWeekOff: original.daysOfWeekOff } })
...
test('settings: trainer has no menu item, read-only form and API rejects PATCH', async ({ page, request }) => {
  ...
  try {
    ...
    await expect(page.getByLabel('Выходные дни', { exact: true })).toBeDisabled()
